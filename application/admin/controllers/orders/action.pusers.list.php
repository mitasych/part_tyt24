...
$list->addWhere('(A.is_deleted = 0 AND NOT EXISTS (SELECT B.user_id from orders_relations AS B WHERE B.user_id = A.id) AND (A.balans > 0 OR A.dogovor_created > 0))');

$lister = new AK_Data_Lister($list, $this->getRequest(), 'pusers');
...
$lister->PARAM_SCRIPT = MODULE_URL.'/orders/pusers.list/';
