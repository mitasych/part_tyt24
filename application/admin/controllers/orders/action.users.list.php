...
$list->addWhere('(A.is_deleted = 0 AND NOT EXISTS (SELECT B.user_id from orders_relations AS B WHERE B.user_id = A.id) AND ( A.balans = 0 OR A.balans IS NULL) AND A.dogovor_created = 0)');

$lister = new AK_Data_Lister($list, $this->getRequest(), 'users');
...
$lister->PARAM_SCRIPT = MODULE_URL.'/orders/users.list/';
...
$_field = new AK_Data_Lister_Field();
$_field->setSQL_FIELD('createDateFormat')->setTEXT_LABEL('Дата регистрации')->setSORTING(true)->setSORT_FIELD('A.create_date');
$lister->addField($_field);
