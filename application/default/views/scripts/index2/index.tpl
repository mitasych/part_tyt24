                ...
                <ul class="brunch">
                    ...
                    {foreach from=$okved item=current name="okved"}
                        ...
                        {assign var="style_menu" value=""}
                          <li>
                            <a style="font-weight:bold; font-size:12px;" href="{$SITE_URL}/database/show/id/{$current.id}/">{$current.name}</a>{if count($current.subitems)>2 and empty($okved_id) and not $all_okved}<a class="drill_down_menu"><img src="{$IMG_URL}/drill_down.jpg" /></a>{/if}
                            <ul class="last_ul_li">
                                ...
                                {if $subitem.count != 0}
	                                ...
	                                {/if}
	                                    <li style="{if $style_menu}{$style_menu}{/if}">
	                                            <a style="font-weight:bold; font-size: 12px;" href="{$SITE_URL}/database/show/id/{$subitem.id}/">{$subitem.name}{if empty($okved_id)}{else}({$subitem.count|number_format:0:".":" "}){/if}</a>{if count($subitem.subitems)>5 and empty($okved_id) and not $all_okved}<a class="drill_down_submenu"><img src="{$IMG_URL}/drill_down_red.jpg" /></a>{/if}
	                                            <br/>
	                                            <ul>
	                                            {assign var="style_submenu" value=""}
	                                                {foreach from=$subitem.subitems item=subsubitem name="third"}
	                                                    {if $subsubitem.count != 0}
	                                                        ...
	                                                        {/if}
	                                                            <a style="{if $style_submenu}{$style_submenu}{/if}" href="{$SITE_URL}/database/show/id/{$subsubitem.id}/">{$subsubitem.name}{if empty($okved_id)}{else} ({$subsubitem.count|number_format:0:".":" "}){/if}</a>
	                                                    {/if}
	                                                {/foreach}
	                                            </ul>
	                                            <br />
...
<!-- rubric -->
      ...
      <td class="region_li2" style='vertical-align: top;'>
            ...
            <div class="branch_submenu">
                <p style="color:#FF332D;">ТОП: {if $max_okato.count neq 0}<a href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/{$max_okato.id}/id/{$id}">{$max_okato.name} ({$max_okato.count|number_format:0:".":" "}) </a><a class="{if $all_okato}li_ico_up{else}li_ico{/if}" href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/{if !$all_okato}all{/if}/id/{$id}">&nbsp;&nbsp;&nbsp;&nbsp;</a>{else}<a href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/12530/id/{$id}">Москва</a>; <a href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/11600/id/{$id}/">Санкт-Петербург</a> <a class="{if $all_okato}li_ico_up{else}li_ico{/if}" href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/{if !$all_okato}all{/if}/id/{$id}">&nbsp;&nbsp;&nbsp;&nbsp;</a>{/if}</p>
            </div>
            {if $current_okato}
            <div class="branch_submenu" style="border:none">
            ...
            <ul class="brunch">
                ...
                {/if}
                <li>
                    {if $current.id}<a style="font-weight:bold; font-size:13px;" href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/{$current.id}/id/{$id}/" title="{$current.name}{if $current.additional_info}, центр: {$current.additional_info}{/if}">{$current.name}</a>{if count($current.subitems)>5 and empty($okato_id) and not $all_okato}<a class="drill_down_okato"><img src="{$IMG_URL}/drill_down_red.jpg" /></a>{/if}<br />
                    {else}{if $current_okato}<a style="font-weight:bold; font-size:13px;" href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/{$current_okato.id}/id/{$id}/" title="{$current_okato.name}{if $current_okato.additional_info}, центр: {$current_okato.additional_info}{/if}">{$current_okato.name}</a><br />{/if}
                    {/if}
                    <ul>
                        ...
                        {if $subitem.count !=0 }
	                        <li style="{if $style_okato}{$style_okato}{/if}">
	                            <a href="{$SITE_URL}/index2/index/okved/{if $all_okved}all{else}{$okved_id}{/if}/okato/{$subitem.id}/id/{$id}/" title="{$subitem.additional_info}">{$subitem.name} {$subitem.type}</a>
	                        </li>
