...
$(document).ready(function() {
	...
	$('.info_image').mouseout(function (){
				$('#description').hide();
	});
	$('.info_image').mouseover(function (kmouse){
			id = $(this).attr('val2');
			base_h = kmouse.pageY - 430;
			base_w = kmouse.pageX - 300;
			var id_el = '#description';
...
$('.111').mouseover(function (kmouse){
			id = $('.111').attr('val2');
			base_h = kmouse.pageY - 430;
			base_w = kmouse.pageX - 300;
			var id_el = '#description';
				$.get('/monitoring/tarifinfo', {tarif_id : $('.111').attr('val')}, function(data){
					$('#conteiner_info').html(data);
				});
				$(id_el).addClass('events_info_block');
				$(id_el).css('top', base_h).css('left', base_w);
...
$('#findCompanyLink').click(function(){
	if ($('#add_comp_monitoring').css('display') == 'block'){
                $('#addCompanyLink'+' a').css({'color':'#2D96FE','text-decoration': 'underline','font-weight':'normal'});
                 $('#addCompanyLink'+' span img').attr('src','/images/add_comp_img.png'); 
                $('#add_comp_monitoring').hide();
                }
        if ($('#full_search').css('display') == 'block'){ 
                $('#findCompanyLink'+' span img').attr('src','/images/drill_right.jpg');
                $('#findCompanyLink'+' a').css({'color':'#2D96FE','text-decoration': 'underline','font-weight':'normal'});
                $('#full_search').hide();
        }
        else {
                $('#findCompanyLink'+' span img').attr('src','/images/drill_up_red.jpg');
                $('#findCompanyLink'+' a').css({'color':'#FF332D','text-decoration': 'none','font-weight':'bold'});  
                $('#full_search').show();
       }
});
...
$('#addCompanyLink').click(function(){
	if ($('#full_search').css('display') == 'block'){
       ...
       if ($('#add_comp_monitoring').css('display') == 'block'){
                ...
                $('#addCompanyLink'+' span img').attr('src','/images/drill_up_red.jpg'); 
                $('#addCompanyLink'+' a').css({'color':'#FF332D','text-decoration': 'none','font-weight':'bold'});  
                $('#add_comp_monitoring').show();
           }
});
	});

var p; 
function changeForm(elem,fullForm){
	if(p == false || p == undefined){
		$(elem+' span img').attr('src','/images/drill_up_red.jpg');
                $(elem+' a').css({'color':'#FF0000','text-decoration': 'none','font-weight':'bold'});
		p = true;
	}else{
		$(elem+' span img').attr('src','/images/drill_right.jpg');
                $(elem+' a').css({'color':'#2D96FE','text-decoration': 'underline','font-weight':'normal'});
		p= false;
	}
	fullForm.toggle();
}
...
{/literal}
<div align="left" id="formEdit" class="events_info_block" style="display:none; position:absolute; left:50%; top:10%; z-index:10000; padding-right: 30px;font-weight: 700; font-size: 12px;">
	<div style="float:right;cursor:pointer;font-size: 15px; font-weight: normal; color: gray;margin-top: 10px; margin-right: -15px;" onclick="$('#formEdit').hide();">X</div>
	<form method="post" action="">
                <p style="font-size: 13px;"> Редактирование компании: </p>
                <label class="style_input">Наименование компании:<br>
		<input id='name_ed' type="text" name="title" value="" style="color: #999;"></label><br><br>
		<label  class="style_input">ИНН:<br>
		<input id='inn_ed' type="text" name="inn" value="" style="color: #999;"></label><br><br>
		<label class="style_input">Регион:<br>
		<input id='reg_ed'  type="text" name="region" value=""></label><br><br>
			...
			{/foreach}
                ...
                <span class="show_tarif" style="float:right;cursor:pointer;margin-top: -20px;margin-right: 70px;"  val="{$user->getActualTarifInfo()->id}"                  			val2="{$user>getTarifId($user->getActualTarifInfo()->id)}"> <img class="db_note" src="/img/db_info.png"></span>
               	<span class="check_exel"><input type="checkbox" name="copy" id="form_copy"> 
                <label class="style_input" for="form_copy" style="font-weight: normal;">Копировать в новую услугу:</lable></span><br>
                <br>
		<label  class="style_input">Адрес:<br>
		<input id='address_ed' type="text" name="address" value=""></label><br><br>
		<label class="style_input">ФИО директора:<br>
		<input id='fio_ed' type="text" name="fio" value=""></label><br><br>
		<label class="style_input">Отрасль:<br>
		<input id='otrasl_ed'  type="text" name="otrasl" value=""></label><br><br>

		
		<input  type="hidden" id="hid_id" name="hid_id" value="">	
		<input  type="hidden" id="hid_company_id" name="hid_company_id" value="">
		<span id="addx3">
...
<div>
    <div class="main_top_text">
        ...
        <div>
			...
			<p>
			<div>
			<span class='main_t ' name="allTarif" id='' style="cursor:pointer;"  onClick="doSearch(arguments[0]||event);filter_all='on'; $('#hide_country_radio').show();">Все услуги</span><span id="rightcomp" class="rightcomp_disact"></span>
			...
			<table style="width: 100%;">
                        <tr>
                        <td>
                        <span id="findCompanyLink" style="float: left;">
                             <span> <img src="/images/drill_right.jpg"/> </span>
			      <a class="l1" href="javascript:void(0);">Найти компанию</a> 
                        </span>
                        </td>
			...
			<script type="text/javascript">
				function selectTarif(){
					...
					id_t=$('#act_tarifs').val();
					console.log('wwww');
					$('#info_image').attr('val2',$('option[value='+$('#act_tarifs').val()+']').attr('id_val'));				
				}
			</script>
			{/literal}
          {if count($userTarifs) > 0}
                        <div id="tarifs" class="style-select" style="width: 100px;">                     
                         <select name="k_id" style="width: 140px;">
                     	 <optgroup>
			{foreach from=$userTarifs item=item}
			<option value="{$item->id}">{$item->count}-{$item->m}-{$item->period}</option>
                        <input id="hiddenId" type="hidden" val="{$item->tarifId}">
			{/foreach}
			</optgroup>
		        </select>
                
			</div>
               <span class="info_image" style="float:right;cursor:pointer;margin-top: -20px;margin-right: 70px;"  val="$('#tarifs').attr('val');"                  			val2="$('#hiddenId').attr('val');"> <img class="db_note" src="/img/db_info.png"></span>
...
{if $kc != $user->getCountMon()}  
                        <span id="addCompanyLink" style="float: right;">
                        <span> <img style="margin-bottom: -3px;" src="/images/add_comp_img.png"/> </span>
                        <a class="l2" href="javascript:void(0);" > Добавить компанию</a>
                        </span>
{/if}                  </td>
			</tr>
		       </table>
			...
			<div  id="add_comp_monitoring">
			<!-- <font color="#1F5763">Добавление компании:</font><br> -->
            <p>
            <table>
	            <tr>
	            	<td><font style="font-size:14px;font-weight:700;">Добавить компанию:</font></td>
	            </tr>	
	            <tr>
...
{/if}		
		...
		<br>
			...
			<font style="font-size:14px;font-weight:700;">Поиск:</font>
				<table cellpadding="5">
					<tr>					
						<td colspan="4">
							{if $countTarif>1} 
					...
					<tr>
						...
						<td>
						 ...
						 <div id="change_tarif" class="style-select" style="display:none; float:left; width: 130px; margin-right: 10px;padding:4px;">
                                                     <select placeholder="Выбрать услугу" name="tarif_id_one" style="width: 150px;color: #999;" >
                                                         <optgroup>
                                                         <option value='0' {if $filter_region == '0'}selected{/if} >Выбрать услугу</option>
                                                         {foreach from=$userTarifs item=item}
				                        <option value="{$item->id}">{$item->count}-{$item->m}-{$item->period}</option>
						         {/foreach}
						          </optgroup>
                                                       ...
                                                       <div class="style_input" style="float:left;">
							<input name='filter_name' placeholder="Наименование" id='filter_name' type='text' value='{$filter_name}' onkeydown="doSearch(arguments[0]||event)">           </div>
						</td>
						...
						<td>
							<div class="style-select" style="width: 150px;"> 
								<select placeholder="Регион"  name='filter_region' style="width: 160px;" id='filter_region' onChange="doSearch(arguments[0]||event)">
									<optgroup>
									<option value='0' {if $filter_region == '0'}selected{/if} >Регион</option>
						...
						<td>
							<a href="#" id="list_clean_button" onClick='javascript:document.forms.SearchCompany.reset(); doSearch(arguments[0]||event); return false'></a>
						</td>
					</tr>
...
<span id='option1'  onClick="$('#option').show();$('#option1').hide();$('#option2').show();" > <img src="/images/drill_right.jpg"/> 
                        <a href="javascript:void(0)"> настройка полей </a> </span> 
                    <span id='option2'  style="display:none;" onClick="$('#option').hide();$('#option2').hide();$('#option1').show();" > <img src="/images/drill_up_red.jpg"/> 
                        <a href="javascript:void(0)"> настройка полей </a> </span>
 ...
 {literal}
		...
		<div class="events_info_block" id="items_sel" style="position:relative;left:70px;top:-30px;display:none;width:250px;">
	   ...
	   <div style="margin-top:-25px;float:right;">Услуга 
...
<span class="radio_style">
        ...
        doSearch(arguments[0]||event);"><label color="#1F5763" for="country_ua">Украина <img src="/images/252.gif"></label></span><br>
            ...
            <div style="float:left; margin-top:-15px;padding:0px;">
                <br>
           <div id="mailSettings"> 
                <span onClick="changeForm('#mailSettings',$('#mails'))"> <img src="/images/drill_right.jpg"/> </span> 
                <a href="javascript:void(0);"  onClick="changeForm('#mailSettings',$('#mails'))"> Настройка рассылки уведомлений</a> <strong> &nbsp;(Общий  адрес</strong>: {$user->getEmail() }) <br>
           </div>
				{* <a href="#" id="mail_change" style="color:#da114b"> изменить </a>
                                 <a href="" id="mail_hide" style="color:#da114b;display:none;"> свернуть </a> *}
			    
				<div id="mails" style="display:none" class="style_input"> 				
                {foreach from=$ttList item=et}
				
				{ if ($et->title =="суд" ) }
				<input type="text" name="court"  value="{$user->getCourtMail() }" class="list_event_input"  onKeydown="$('#save_mail').attr('data-id','1').attr('class','file_input_button')"/>
                <span style="width:20px; height:20px;background-color:{$et->getColor()}">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;
				    <b style="color:{$et->getColor()}"> {$et->title|escape}</b> {/if}
					
				{ if ($et->title =="егрюл" ) }
				<input type="text" name="egrul" value="{$user->getEgrulMail()}" class="list_event_input" onKeydown="$('#save_mail').attr('data-id','1').attr('class','file_input_button')"/> 
                <span style="width:20px; height:20px;background-color:{$et->getColor()}">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;
				    <b style="color:{$et->getColor()}"> {$et->title|escape}</b> {/if}

				{ if ($et->title =="банкротство" ) }
                <input type="text" name="bankruptcy" value="{$user->getBankMail() }"  class="list_event_input" onKeydown="$('#save_mail').attr('data-id','1').attr('class','file_input_button')"/>
                <span style="width:20px; height:20px;background-color:{$et->getColor()}">&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;
				    <b style="color:{$et->getColor()}"> {$et->title|escape}</b> {/if}					
	            <br/>		
                {/foreach}
				...
				<span style="display:none;color:#da114b;" id="saved" > изменения сохранены <font color="#DBA108"></font>  </span></div>
			</p>
...
<div style="z-index:100500;position:relative;float:right;cursor:pointer;font-size: 15px;top: -665px; right: 0px;">

                    {* <br><span id='option_l'  onclick="$('#option_l').hide(); $('#option').show(); " class="file_input_button"> Настройки </span>       *}       
        <div class="events_info_block" id="option" style="margin-left: -170px;display:none;position:absolute;cursor:pointer;font-size: 15px;top:65%;left:-50%;padding-left:15px; width:150px;">
            ...
            <input class="type_select" name="title" type="checkbox"  style="width:10px;"  id="hide_title" value="checked"   {$datas.title}/>
                ...
                <label  for="hide_otrasl"  ></label>Отрасль</span><br>
        		 ...
        		 <input type="submit" name="save_setting" value="Сохранить"  class="file_input_button">
                </form>
