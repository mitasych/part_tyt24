...
{/literal}
<div align="right" id="formEdit" class="events_info_block" style="display:none; position:absolute; left:350px; top:400px; z-index:10000; padding-right:30px"><br>
	<div style="float:right;cursor:pointer;font-size: 15px;color: gray;margin-top: -5px;" onclick="$('#formEdit').hide();">X</div><br>
	<form method="post" action="">
		...
		<span id="addx3">
		<input type="submit" value="Сохранить изменения"></span>
	</form>
 ...
 {literal}
            ...
            <script type="text/javascript">
			 ...
			 jQuery("#bigset").jqGrid({
                url:'/monitoring/kontrdatagroup/?tarif={/literal}{$current_mon_tarif}{literal}',
                datatype: "json",
               	height: 100+"%",
                width: 90+"%",
                colNames:['Регион','Страна'],
                    colModel :[
             ...
             autowidth: true,
                ...
                pgtext: false,
                //  subGrid : true, 
                // subGridUrl: '/monitoring/kontrdatagroup/?tarif={/literal}{$current_mon_tarif}{literal}', subGridModel: [{ name : ['№', 'Наименование', 'ИНН/ЕДРПОУ','События','Адрес','Фио директора','Дата регистрации','Отрасль', 'Действия'], width : [50,150,230,180] } ],
                pginput:false,
                multiselect: true,
  		...
  		// 		var filter_all = this.value
                ...
                jQuery("#bigset").jqGrid('setGridParam',{url:"/monitoring/kontrdatagroup/?filter_region="+filter_region+"&tarif={/literal}{$current_mon_tarif}{literal}&filter_name="+filter_name
                	+"&filter_inn="+filter_inn+"&filter_all="+filter_all+"&country_mask="+country_mask}).trigger("reloadGrid");
                   delete filter_all;   
		...
		$('#jqgh_del').live('click', function(){
			...
			<div style="margin-top:10px; margin-left:10px; position:relative"><a style="color:#808080; " href="#" onClick="javascript:$('input:checkbox').attr('checked','checked');return false;">(Выделить все</a>/
			<a style="color:#808080;" href="#" onClick="javascript:$('input:checkbox').removeAttr('checked'); return false;">Отметь все)</a>

			 <a href="javascript:void(0);" id="m1">Удалить выбранные</a>
		</div>
...
{/literal}
		...
		<label><input id='is_copy' value='1' type="checkbox" name="copy" id="form_copy"> Копировать в новую услугу:</lable><br>
{literal}
		</div>
...
<span class="radio_style">
        ...
        doSearch(arguments[0]||event);"><label color="#1F5763" for="country_ua">Украина <img src="/images/252.gif"></label></span><br>
			...
			<div style="z-index:100500;position:relative;float:right;cursor:pointer;font-size: 15px;top:-200px;right:5px">
