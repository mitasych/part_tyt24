<div class="right_part2">
    ...
    <div>
        <div class="main_top_text">
            ...
            <h1>{if $nn.0->zakazSubtypeId !=21}Заказ принят{else}Пополнение баланса{/if}</h1>
            ...
            {else}
                ...
                <br><a href="{$SITE_URL}/order/balans_show/sc/{$sc}/">Перейти на страницу заказа</a>
                </p>
                {else}
                <p>На Ваш почтовый ящик отправлен счет/квитанция для пополнения баланса. Статус оплаты Вы можете проверить
                    <a href="{$SITE_URL}/order/showbalans/sc/{$sc}/">здесь</a>
                </p>
                {/if}
            {/if}
