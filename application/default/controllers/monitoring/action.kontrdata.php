...
//if (!empty($this->params['filter_serch']))
{
	...
	if (!empty($this->params['filter_region']))
	{
     ...
     if (!empty($_GET['country_mask'])) {
        ...
        $kontragentsList->addWhere("A.favorites LIKE ?", '1'.'%');
    }
}
...
foreach($kontragentsList as $item)
{
	...
	$item_new['id']=$item->id;
        ...
        if ($item->favorites == 1){
            $favorites = '<span title="В избранном" style="display:block" class="favorites-star" id="f_'.$item->id.'" onClick="$(\'#infor\').hide(); favor(0,'.$item->id.')" onmouseover="$(\'#info_favor\').show();"
            onmouseout="$(\'#info_favor\').hide();"><span>';



    } else {
            $favorites = '<span title="В избранное" style="display:block" class="nofavorites-star" id="f_'.$item->id.'"  onClick="$(\'#infor\').hide(); favor(1,'.$item->id.')"  onmouseover="$(\'#info_favor1\').show();"
	...
	$item_new['cell']=array($favorites,$i, $title, $item->getKontragent()->inn, $item->getKontragent()->region, 

      "<a href ='/monitoring/events/filter/all?inn=".$item->getKontragent()->inn."'>" .   $countContr[$item->kontragentId]['count'] ."</a>",
         $item->getKontragent()->country,
         $item->getKontragent()->adress,
         $item->getKontragent()->rykov,
         $item->getKontragent()->reg_date,
         $item->getKontragent()->otrasl,
        '<a href="/reports/order/cargoname2/'.$item->getKontragent()->inn.'/cargoname/'.$item->getKontragent()->title.'">
        ...
        <a href="/monitoring/list/did/'.$item->id.'/" data='.$item->id.'
        onClick="return confirmDelete()"><img src="/images/delete_company.png" title="Удалить" /></a>
        ');
	/*$item_new['name']='del';
	$item_new['inn']='del';
	$item_new['region']='del';
