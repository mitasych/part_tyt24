...
//print_r($this->view->periodList); exit();
...
if ( empty($this->params['refresh'])  && $form->validate($this->params)) {
    ...
    $item->m = $parts[1]*30;	
	$country_name = new AK_Location_Country($this->params['country_tarif']);
	$item->mon_count = $mon_count;
	$item->country_name = $country_name->name;
	$item->country_id = $this->params['country_tarif'];
	$item->mon_discount = $discount;
	$item->mon_price = $price;
	$item->new_tarif = 1;
    $item->price_one = $this->params['price_one'];
    $item->all_id = $this->params['all_id'];
