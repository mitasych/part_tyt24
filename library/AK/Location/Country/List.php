...
class AK_Location_Country_List extends AK_Order_Abstract_List
{
 ...
 /**
    ...
    public function getList()
    {
    	...
    	if ( $this->isAsAssoc() ) {
            ...
            $query->from('orders_location__countries AS A', $fields);	
    	} else {
    	    $query->from('orders_location__countries AS A', 'A.*');
    	}
        ...
        if ( $this->isAsAssoc() ) {
	        ...
	        foreach ( $items as &$item ) $item = new AK_Location_Country($item);
        }*/
    ...
    public function setCount($value=null) {
      ...
      if (empty($value)) {
        $query = $this->_db->select();
        $query->from('orders_location__countries AS A', 'COUNT(A.id)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
