...
class AK_Menu_Sublink_List extends AK_Abstract_List
{
 ...
 /**
    ...
    public function getList()
    {
    	...
    	if ( $this->isAsAssoc() ) {
            ...
            $query->from(DBT_PREFIX.'_menus__items_sublinks AS A', $fields);	
    	} else {
    	    $query->from(DBT_PREFIX.'_menus__items_sublinks AS A', 'A.*');
    	}
        ...
        if ( $this->isAsAssoc() ) {
	        ...
	        foreach ( $items as &$item ) $item = new AK_Menu_Sublink_Item($item);
        }
        return $items;
    }
    ...
    public function setCount($value=null) {
      ...
      if (empty($value)) {
        $query = $this->_db->select();
        $query->from(DBT_PREFIX.'_menus__items_sublinks AS A', 'COUNT(A.id)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
