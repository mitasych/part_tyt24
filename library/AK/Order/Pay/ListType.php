...
class AK_Order_Pay_ListType extends AK_Order_Abstract_List
{
 ...
 /**
    ...
    public function getList()
    {
    	...
    	if ( $this->isAsAssoc() ) {
            ...
            $query->from('order_pay_type AS A', $fields);	
    	} else {
    	    $query->from('order_pay_type AS A', 'A.*');
    	}
        ...
        if ( $this->isAsAssoc() ) {
	        ...
	        foreach ( $items as &$item ) $item = new AK_Order_Pay_ItemType($item);
        }
    ...
    public function setCount($value=null) {
      ...
      if (empty($value)) {
        $query = $this->_db->select();
        $query->from('order_pay_type AS A', 'COUNT(A.id)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
