...
class AK_Order_Prices extends AK_Data_EntityOrder {
    ...
    public function __construct ($value = null, $off = 0, $isoff = 0, $region_code = 0) {
		...
		$list_ofreport = new AK_Order_Report_Oflist();
		
		if($region_code > 0 && $this->id > 0){
			$this->ofReport = $list_ofreport->getOfreportByCodeAndReport($region_code, $this->id);
		}
		//echo '<b>'.__FILE__.' -- '.__LINE__.'</b><pre>'; var_dump($this->ofReport); echo'</pre>';die;
		
		if ($off == 0)
			$this->price = $this->report->price;
		elseif ($isoff == 0)
// 			$this->price = $this->report->price2;
			$this->price = $this->ofReport->getPriceFront(1) ? $this->ofReport->getPriceFront(1) : 0;
		else
// 			$this->price = $this->report->price3;
			$this->price = $this->ofReport->getPriceFront(2) ? $this->ofReport->getPriceFront(2) : 0;
//     	echo '<b>'.__FILE__.' -- '.__LINE__.'</b><pre>'; var_dump($this->price); echo'</pre>';die;
