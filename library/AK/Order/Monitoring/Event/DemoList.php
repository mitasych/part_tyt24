...
class AK_Order_Monitoring_Event_DemoList extends AK_Order_Abstract_List {
    ...
    public function getList() {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from('orders_monitoring__demoevents AS A', $fields);
        }
        else {
            $query->from('orders_monitoring__demoevents AS A', 'A.*');
        }
        ...
        if ( $this->isAsAssoc() ) {
            ...
            foreach ( $items as &$item ) $item = new AK_Order_Monitoring_Event($item);
        }
    ...
    public function setCount($value=null) {
        ...
        if (empty($value)) {
            $query = $this->_db->select();
            $query->from('orders_monitoring__demoevents AS A', 'COUNT(A.id)');
            if ( $this->getWhere() ) $query->where($this->getWhere());
