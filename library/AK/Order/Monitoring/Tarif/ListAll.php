...
class AK_Order_Monitoring_Tarif_ListAll extends AK_Order_Abstract_List {
    ...
    public function getList() {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from('orders_monitoring__tarif_all AS A', $fields);
        }
        else {
            $query->from('orders_monitoring__tarif_all AS A', 'A.*');
        }
        ...
        if ( $this->isAsAssoc() ) {
            ...
            foreach ( $items as &$item ) $item = new AK_Order_Monitoring_TarifAll($item);
        }
    ...
    public function setCount($value=null) {
        ...
        if (empty($value)) {
            $query = $this->_db->select();
            $query->from('orders_monitoring__tarif_all AS A', 'COUNT(A.id)');
            if ( $this->getWhere() ) $query->where($this->getWhere());
	...
	* @voodoo
	...
	public function getMinimal(){
	  $query = $this->_db->select();
	  $query->from('orders_monitoring__tarif_all AS A','A.num')->where("type = 'Minimal' ");
    ...
    public function getTestTarifCount() {
	  $query = $this->_db->select();
	  $query->from('orders_monitoring__tarif_all AS A','A.num')->where("type = 'Test' ");
	  $result = $this->_db->fetchOne($query);
	  return $result;     
    }	
