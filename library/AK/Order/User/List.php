...
class AK_Order_User_List extends AK_Order_Abstract_List
{
 ...
 /**
    ...
    public function getList()
    {
    	...
    	if ( $this->isAsAssoc() ) {
            ...
            $query->from('orders_users__accounts AS A', $fields);	
    	} else {
    	    $query->from('orders_users__accounts AS A', 'A.*');
    	}
        ...
        if ( $this->isAsAssoc() ) {
	        ...
	        foreach ( $items as &$item ) $item = new AK_Order_User($item);
        }
    ...
    public function setCount($value=null) {
      ...
      if (empty($value)) {
        $query = $this->_db->select();
        $query->from('orders_users__accounts AS A', 'COUNT(A.id)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
