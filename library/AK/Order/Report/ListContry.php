...
class AK_Order_Report_ListContry extends AK_Order_Abstract_List
{
 ...
 /**
    ...
    public function getList()
    {
    	...
    	if ( $this->isAsAssoc() ) {
            ...
            $query->from('orders_country AS A', $fields);	
    	} else {
    	    $query->from('orders_country AS A', 'A.*');
    	}
    ...
    public function setCount($value=null) {
      ...
      if (empty($value)) {
        $query = $this->_db->select();
        $query->from('orders_country AS A', 'COUNT(A.id)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
